...
def test_micro_gripper():
    """Test micro parallel gripper implementation"""
    p.connect(p.DIRECT)
    ...
    # Load micro gripper
    gripper_urdf = os.path.join(BASE_DIR, "urdf", "micro_parallel_gripper.urdf")
    ...
    print(f"Micro gripper joints: {gripper_joints}")
    ...
    def close_gripper():
        for joint_idx in gripper_joints:
            p.setJointMotorControl2(
                ...
                joint_idx,
                p.POSITION_CONTROL,
                targetPosition=0.008,  # 8mm max
                force=8,  # Very gentle force
                positionGain=0.3  # High gain for precision
            )
        print("Micro gripper closed")
    ...
    grasp_position = [cube_pos[0], cube_pos[1], cube_pos[2] + 0.02]  # Much closer to cube
    ...
    grasp_success = False
    if final_contacts > 0:
        print("SUCCESS: Micro gripper in contact with cube!")
        ...
        if lift_height > 0.05:  # 5cm minimum lift
            ...
            if place_accuracy < 0.05:  # 5cm accuracy
                ...
                print("FINAL RESULT: MICRO GRIPPER PICK AND PLACE COMPLETE!")
                success = True
            else:
    ...
    if success:
        print("\nMICRO GRIPPER TEST: PASSED!")
    else:
        print("\nMICRO GRIPPER TEST: FAILED!")
