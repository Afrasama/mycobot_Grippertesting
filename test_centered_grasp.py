...
def test_centered_grasp():
    """Test cube centering between gripper claws"""
    p.connect(p.GUI)  # Use GUI to visualize centering
    ...
    # Load guaranteed gripper with updated centering
    gripper_urdf = os.path.join(BASE_DIR, "urdf", "guaranteed_gripper.urdf")
    ...
    print(f"Centered gripper joints: {gripper_joints}")
    ...
    print("Centered gripper attached")
    ...
    def close_gripper():
        for joint_idx in gripper_joints:
            p.setJointMotorControl2(
                ...
                joint_idx,
                p.POSITION_CONTROL,
                targetPosition=0.006,  # 6mm for proper centering
                force=5,
                positionGain=0.3
            )
        print("Centered gripper closed - cube centered")
    ...
    # Calculate positions for centered approach
    pick_position = [cube_pos[0], cube_pos[1], cube_pos[2] + 0.12]
    grasp_position = [cube_pos[0], cube_pos[1], cube_pos[2] + 0.008]  # 8mm for centering
    place_position = [0.10, 0.30, 0.20]
    
    print(f"Cube at: {cube_pos}")
    print(f"Pick position: {pick_position}")
    print(f"Centered grasp position: {grasp_position}")
    ...
    # Step 3: Close gripper with centering
    print("Closing centered gripper...")
    close_gripper()
    
    # Monitor contacts during closing with centering verification
    print("Monitoring contacts for centered grasp...")
    contact_history = []
    for i in range(100):  # More time for centering
        p.stepSimulation()
        contacts = p.getContactPoints(gripper, cube)
        contact_history.append(len(contacts))
        if i % 20 == 0:
            print(f"  Step {i}: {len(contacts)} contacts")
            # Check contact distribution for centering
            if len(contacts) > 0:
                left_contacts = sum(1 for c in contacts if c[2] == 0)  # Left finger
                right_contacts = sum(1 for c in contacts if c[2] == 1)  # Right finger
                print(f"    Left finger contacts: {left_contacts}")
                print(f"    Right finger contacts: {right_contacts}")
    ...
    # Step 4: Check centered grasp
    grasp_success = False
    if final_contacts > 0:
        print("SUCCESS: Centered gripper in contact with cube!")
        ...
        grasp_success = True
        print("Centered grasp constraint created!")
        
        # Step 5: Test lift with centering
        print("Testing centered lift...")
        ...
        if lift_height > 0.05:
            print("CENTERED LIFT SUCCESSFUL!")
            ...
            # Step 7: Release
            print("Releasing centered object...")
            open_gripper()
            for _ in range(40):
                p.stepSimulation()
            ...
            if place_accuracy < 0.05:
                print("CENTERED PLACE SUCCESSFUL!")
                print("FINAL RESULT: CUBE PROPERLY CENTERED AND PICKED!")
                success = True
            else:
                print("PLACE ACCURACY POOR")
                success = False
        else:
            print("CENTERED LIFT FAILED - Object not lifted properly")
            success = False
    else:
        print("FAILED: No centered contact with cube")
        success = False
    
    # Keep GUI open for inspection
    print("\nGUI mode: Inspect cube centering...")
    ...
    if success:
        print("\nCENTERED GRASP TEST: PASSED!")
        print("Cube is properly positioned between gripper claws!")
    else:
        print("\nCENTERED GRASP TEST: FAILED!")
        print("Cube centering needs adjustment.")
