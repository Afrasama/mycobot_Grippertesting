...
def test_stable_grasp():
    """Test stable grasp implementation"""
    p.connect(p.DIRECT)
    ...
    # Load micro scale gripper
    gripper_urdf = os.path.join(BASE_DIR, "urdf", "micro_scale_gripper.urdf")
    ...
    print(f"Micro scale gripper joints: {gripper_joints}")
    
    # Attach gripper with high stability
    ...
    p.changeConstraint(constraint, maxForce=300)  # High stability
    print("Micro scale gripper attached with high stability")
    ...
    def open_gripper():
        ...
        print("Micro scale gripper opened")
    ...
    def close_gripper():
        for joint_idx in gripper_joints:
            p.setJointMotorControl2(
                ...
                joint_idx,
                p.POSITION_CONTROL,
                targetPosition=0.002,  # 2mm micro scale
                force=1,
                positionGain=0.3
            )
        print("Micro scale gripper closed")
    ...
    grasp_position = [cube_pos[0], cube_pos[1], cube_pos[2] + 0.001]  # 1mm micro scale
    
    print(f"Cube at: {cube_pos}")
    print(f"Pick position: {pick_position}")
    print(f"Stable grasp position: {grasp_position}")
    ...
    # Step 3: Close micro scale gripper
    print("Closing micro scale gripper for stable grasp...")
    close_gripper()
    
    # Monitor contacts for stable grasp
    print("Monitoring contacts for stable grasp...")
    grasp_stable = False
    contact_count = 0
    stable_contacts = 0
    contact_history = []
    
    for i in range(25):  # More steps for stable grasp
        p.stepSimulation()
        contacts = p.getContactPoints(gripper, cube)
        contact_history.append(len(contacts))
        if len(contacts) > 0:
            contact_count += 1
            if len(contacts) >= 2:  # Need multiple contact points for stability
                stable_contacts += 1
            if stable_contacts >= 5:  # Need 5 stable contact checks
                grasp_stable = True
                print(f"    ✅ STABLE GRASP ACHIEVED at step {i}!")
                break
        if i % 5 == 0:
            print(f"  Step {i}: {len(contacts)} contacts, stable_checks: {stable_contacts}")
    ...
    print(f"Stable contacts: {stable_contacts}")
    
    # Step 4: Check grasp
    grasp_success = False
    if grasp_stable:
        print("SUCCESS: Stable grasp achieved!")
        ...
        p.changeConstraint(grasp_constraint, maxForce=200)  # High force for stability
        
        grasp_success = True
        print("Stable grasp constraint created with high force!")
        
        # Step 5: Test lift with stability
        print("Testing stable lift...")
        ...
        if lift_height > 0.05 and len(contacts_after_lift) > 0:
            print("STABLE LIFT SUCCESSFUL!")
            print("✅ Cube securely held between claws!")
            success = True
        else:
            print("STABLE LIFT FAILED - Object unstable")
            success = False
    else:
        print("FAILED: No stable grasp achieved")
        success = False
    
    p.disconnect()
    
    if success:
        print("\nSTABLE GRASP TEST: PASSED!")
        print("Cube is securely positioned between gripper claws!")
    else:
        print("\nSTABLE GRASP TEST: FAILED!")
        print("Stable grasp needs further adjustment.")
